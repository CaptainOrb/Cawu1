...
void gameScreen(int score, int *ptrPlayerX, int *ptrPlayerY, int foodX[], int foodY[])
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        for (int j = 0; j < 9; j++)
        {
            if (i == *ptrPlayerY && j == *ptrPlayerX)
            {
                printf("$");
            }
            else if (i == foodY[0] && j == foodX[0])
            {
                printf("*");
            }
            else if (i == foodY[1] && j == foodX[1])
            {
                printf("*");
            }
            else if (i == foodY[2] && j == foodX[2])
            {
                printf("*");
            }
            else
...
void randomFood(int foodPosX[], int foodPosY[])
{
    ...
    do {
        foodPosX[2] = (rand() * 11) % 9;
        foodPosY[2] = (rand() * 13) % 9;
...
int main()
{
    while (true)
    {
        ...
        {
            control = getch();
            if (control == 'w' && playerPosY != 0)
            {
                playerPosY--;
            }
            else if (control == 'a' && playerPosX != 0)
            {
                playerPosX--;
            }
            else if (control == 's' && playerPosY != 8)
            {
                playerPosY++;
            }
            else if (control == 'd' && playerPosX != 8)
            {
                playerPosX++;
            }
            else if (control == 'q')
            ...
            gameScreen(score, ptrPlayerPosX, ptrPlayerPosY, foodPosX, foodPosY);
            // printf("\n%d %d %d %d %d %d\n", foodPosX[0], foodPosY[0], foodPosX[1], foodPosY[1], foodPosX[2], foodPosY[2]);
        }
    }
}